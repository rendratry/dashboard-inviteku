...
function StatCard({
  title, value, icon, colorFrom, colorTo, delay = 0,
}: { title: string; value: string; icon: React.ReactNode; colorFrom: string; colorTo: string; delay?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      <div>
        <p className="text-xs font-medium text-slate-soft uppercase tracking-wider mb-0.5">{title}</p>
...
function QuickAction({
  href, icon, label, description, colorFrom, colorTo, delay = 0,
}: { href: string; icon: React.ReactNode; label: string; description: string; colorFrom: string; colorTo: string; delay?: number }) {
  return (
    <motion.div
